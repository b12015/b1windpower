 ...
 *    real(H)      : Transfer fuction; Complex matrix(NOUTPUT,NINPUT)
 *    imag(H)        with all switches open
 ...
 *
 *   G. Sybille (IREQ) January 2001, 05-jan-2010
 *   C version from original "m" file: R. Roussel (IREQ) May 2003,
 *   Patrice Brunelle, 20-Oct-2010
 *            
...
{
    int_T nInputs      = (int)mxGetN(Hr_PARAM(S));
    int_T nOutputs     = (int)mxGetM(Hr_PARAM(S));
    int_T nSwitch      = (int)mxGetM(Rswitch_PARAM(S));
    ...
    ssSetSFcnParamTunable(S, Hr_IDX, false);
    ssSetSFcnParamTunable(S, Hi_IDX, false);
...
#define MDL_START
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    int_T   nOutputs     = (int)mxGetM(Hr_PARAM(S));
    int_T   nSwitch      = (int)mxGetM(Rswitch_PARAM(S));
    int_T   inzLen       = (int)mxGetN(InputsNonZero_PARAM(S));
    real_T  *RSwitches   = (real_T *)mxGetPr(Rswitch_PARAM(S));
    real_T  *HinitR      = (real_T *)mxGetPr(Hr_PARAM(S));
    real_T  *HinitI      = (real_T *)mxGetPr(Hi_PARAM(S));
    void    **pWork      = NULL;
...
{
    creal_T *H			= (creal_T*)ssGetPWorkValue(S, H_IND);
	int_T   *swStates	= (int_T*)ssGetPWorkValue(S, SW_STATES_IND);
    int_T   *swStates1	= (int_T*)ssGetPWorkValue(S, SW_STATES1_IND);
    int_T   *swStates2	= (int_T*)ssGetPWorkValue(S, SW_STATES2_IND);
    int_T   *swsOn		= (int_T*)ssGetPWorkValue(S, SWS_ON_IND); 
    real_T  *Ysw		= (real_T*)ssGetPWorkValue(S, Y_SWITCHES_IND);
    real_T  *I			= (real_T*)ssGetPWorkValue(S, I_MATRIX_IND);
    creal_T *H1			= (creal_T*)ssGetPWorkValue(S, H1_IND);
    creal_T *H2			= (creal_T*)ssGetPWorkValue(S, H2_IND);
	creal_T *Hsw		= (creal_T*)ssGetPWorkValue(S, HSW_IND);
    creal_T *tmpMat		= (creal_T*)ssGetPWorkValue(S, TMP_MAT_IND);
    creal_T *Hinit      = (creal_T*)ssGetPWorkValue(S, HINIT_IND);
    creal_T *mm_lu		= (creal_T*)ssGetPWorkValue(S, MM_LU_IND);
	int32_T *mm_piv     = (int32_T*)ssGetPWorkValue(S, MM_PIV_IND);
    creal_T *mm_x       = (creal_T*)ssGetPWorkValue(S, MM_X_IND);

    const int_T       nInputs   = ssGetIWorkValue(S,NINPUT);
    const int_T       nOutputs  = ssGetIWorkValue(S,NOUTPUT);
    const int_T       nSwitch   = ssGetIWorkValue(S,NSWITCH);

	/* Create a MATLAB structure to hold the run-time data */
    mxArray* simSnap = mxCreateStructMatrix(1, 1, nFields, fieldNames);
	
	{
		mxArray* mH= mxCreateDoubleMatrix(1,2 * nInputs * nOutputs,mxCOMPLEX);
		memcpy(mxGetPr(mH), &H->re, nInputs * nOutputs*sizeof(real_T));
		memcpy(mxGetPi(mH), &H->im, nInputs * nOutputs*sizeof(real_T));
		mxSetFieldByNumber(simSnap, 0, 0, mH);
	}	
	
	if (nSwitch > 0)
	{		
		{
			mxArray* mswStates= mxCreateDoubleMatrix(1,nSwitch,mxREAL);
			memcpy(mxGetPr(mswStates), swStates, nSwitch*sizeof(int_T));
			mxSetFieldByNumber(simSnap, 0, 1, mswStates);
		}	
		{
			mxArray* mswStates1= mxCreateDoubleMatrix(1,nSwitch,mxREAL);
			memcpy(mxGetPr(mswStates1), swStates1, nSwitch*sizeof(int_T));
			mxSetFieldByNumber(simSnap, 0, 2, mswStates1);
		}	
		
		{
			mxArray* mswStates2= mxCreateDoubleMatrix(1,nSwitch,mxREAL);
			memcpy(mxGetPr(mswStates2), swStates2, nSwitch*sizeof(int_T));
			mxSetFieldByNumber(simSnap, 0, 3, mswStates2);
		}	
		{
			mxArray* mswsOn= mxCreateDoubleMatrix(1,nSwitch,mxREAL);
			memcpy(mxGetPr(mswsOn), swsOn, nSwitch*sizeof(int_T));
			mxSetFieldByNumber(simSnap, 0, 4, mswsOn);
		}	
		{
			mxArray* mYsw= mxCreateDoubleMatrix(1,nSwitch,mxREAL);
			memcpy(mxGetPr(mYsw), Ysw, nSwitch*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 5, mYsw);
		}	
		{
			mxArray* mI= mxCreateDoubleMatrix(1,nOutputs * nOutputs,mxREAL);
			memcpy(mxGetPr(mI), I, nOutputs * nOutputs*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 6, mI);
		}	
		{
			mxArray* mH1= mxCreateDoubleMatrix(1,nInputs * nOutputs,mxCOMPLEX);
			memcpy(mxGetPr(mH1), &H1->re, nInputs * nOutputs*sizeof(real_T));
			memcpy(mxGetPi(mH1), &H1->im, nInputs * nOutputs*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 7, mH1);
		}	
		{
			mxArray* mH2= mxCreateDoubleMatrix(1,nInputs * nOutputs,mxCOMPLEX);
			memcpy(mxGetPr(mH2), &H2->re, nInputs * nOutputs*sizeof(real_T));
			memcpy(mxGetPi(mH2), &H2->im, nInputs * nOutputs*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 8, mH2);
		}	
		{
			mxArray* mHsw= mxCreateDoubleMatrix(1,nInputs * nOutputs,mxCOMPLEX);
			memcpy(mxGetPr(mHsw), &Hsw->re, nInputs * nOutputs*sizeof(real_T));
			memcpy(mxGetPi(mHsw), &Hsw->im, nInputs * nOutputs*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 9, mHsw);
		}	
		{
			mxArray* mtmpMat= mxCreateDoubleMatrix(1,nOutputs * nOutputs,mxCOMPLEX);
			memcpy(mxGetPr(mtmpMat), &tmpMat->re, nOutputs * nOutputs*sizeof(real_T));
			memcpy(mxGetPi(mtmpMat), &tmpMat->im, nOutputs * nOutputs*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 10, mtmpMat);
		}	
		{
			mxArray* mHinit= mxCreateDoubleMatrix(1,nInputs * nOutputs,mxCOMPLEX);
			memcpy(mxGetPr(mHinit), &Hinit->re, nInputs * nOutputs*sizeof(real_T));
			memcpy(mxGetPi(mHinit), &Hinit->im, nInputs * nOutputs*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 11, mHinit);
		}	
		{
			mxArray* mmm_lu= mxCreateDoubleMatrix(1,nOutputs * nOutputs,mxCOMPLEX);
			memcpy(mxGetPr(mmm_lu), &mm_lu->re, nOutputs * nOutputs*sizeof(real_T));
			memcpy(mxGetPi(mmm_lu), &mm_lu->im, nOutputs * nOutputs*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 12, mmm_lu);
		}	
		{
			mxArray* mmm_piv= mxCreateDoubleMatrix(1,nOutputs,mxREAL);
			memcpy(mxGetPr(mmm_piv), mm_piv, nOutputs*sizeof(int32_T));
			mxSetFieldByNumber(simSnap, 0, 13, mmm_piv);
		}	
		
		{
			mxArray* mmm_x= mxCreateDoubleMatrix(1,nInputs * nOutputs,mxCOMPLEX);
			memcpy(mxGetPr(mmm_x), &mm_x->re, nInputs * nOutputs*sizeof(real_T));
			memcpy(mxGetPi(mmm_x), &mm_x->im, nInputs * nOutputs*sizeof(real_T));
			mxSetFieldByNumber(simSnap, 0, 14, mmm_x);
		}
	}
    return simSnap;
}
...
static void mdlSetSimState(SimStruct* S, const mxArray* simSnap)
{
	...
	{
		mxArray* mH = mxGetField(simSnap, 0, fieldNames[0]);
		memcpy(&H->re, mxGetPr(mH), nInputs * nOutputs*sizeof(real_T));
		memcpy(&H->im, mxGetPi(mH), nInputs * nOutputs*sizeof(real_T));
	}	
	if (nSwitch > 0)
	{	
		{
			mxArray* mswStates = mxGetField(simSnap, 0, fieldNames[1]);
			memcpy(swStates, mxGetPr(mswStates), nSwitch*sizeof(int_T));
		}					
		{
			mxArray* mswStates1 = mxGetField(simSnap, 0, fieldNames[2]);
			memcpy(swStates1, mxGetPr(mswStates1), nSwitch*sizeof(int_T));
		}			
		{
			mxArray* mswStates2 = mxGetField(simSnap, 0, fieldNames[3]);
			memcpy(swStates2, mxGetPr(mswStates2), nSwitch*sizeof(int_T));
		}	
		{
			mxArray* mswsOn= mxGetField(simSnap, 0, fieldNames[4]);
			memcpy(swsOn, mxGetPr(mswsOn), nSwitch*sizeof(int_T));
		}	
		{
			mxArray* mYsw= mxGetField(simSnap, 0, fieldNames[5]);
			memcpy(Ysw, mxGetPr(mYsw), nSwitch*sizeof(real_T));
		}	
		{
			mxArray* mI= mxGetField(simSnap, 0, fieldNames[6]);
			memcpy(I, mxGetPr(mI), nOutputs * nOutputs*sizeof(real_T));
		}	
		{
			mxArray* mH1= mxGetField(simSnap, 0, fieldNames[7]);
			memcpy(&H1->re, mxGetPr(mH1), nInputs * nOutputs*sizeof(real_T));
			memcpy(&H1->im, mxGetPi(mH1), nInputs * nOutputs*sizeof(real_T));
		}	
		{
			mxArray* mH2= mxGetField(simSnap, 0, fieldNames[8]);
			memcpy(&H2->re, mxGetPr(mH2), nInputs * nOutputs*sizeof(real_T));
			memcpy(&H2->im, mxGetPi(mH2), nInputs * nOutputs*sizeof(real_T));
		}	
		{
			mxArray* mHsw= mxGetField(simSnap, 0, fieldNames[9]);
			memcpy(&Hsw->re, mxGetPr(mHsw), nInputs * nOutputs*sizeof(real_T));
			memcpy(&Hsw->im, mxGetPi(mHsw), nInputs * nOutputs*sizeof(real_T));
		}	
		{
			mxArray* mtmpMat= mxGetField(simSnap, 0, fieldNames[10]);
			memcpy(&tmpMat->re, mxGetPr(mtmpMat), nOutputs * nOutputs*sizeof(real_T));
			memcpy(&tmpMat->im, mxGetPi(mtmpMat), nOutputs * nOutputs*sizeof(real_T));
		}	
		{
			mxArray* mHinit= mxGetField(simSnap, 0, fieldNames[11]);
			memcpy(&Hinit->re, mxGetPr(mHinit), nInputs * nOutputs*sizeof(real_T));
			memcpy(&Hinit->im, mxGetPi(mHinit), nInputs * nOutputs*sizeof(real_T));
		}	
		{
			mxArray* mmm_lu= mxGetField(simSnap, 0, fieldNames[12]);
			memcpy(&mm_lu->re, mxGetPr(mmm_lu), nOutputs * nOutputs*sizeof(real_T));
			memcpy(&mm_lu->im, mxGetPi(mmm_lu), nOutputs * nOutputs*sizeof(real_T));
		}	
		{
			mxArray* mmm_piv= mxGetField(simSnap, 0, fieldNames[13]);
			memcpy(mm_piv, mxGetPr(mmm_piv), nOutputs*sizeof(int32_T));
		}			
		{
			mxArray* mmm_x= mxGetField(simSnap, 0, fieldNames[14]);
			memcpy(&mm_x->re, mxGetPr(mmm_x), nInputs * nOutputs*sizeof(real_T));
			memcpy(&mm_x->im, mxGetPi(mmm_x), nInputs * nOutputs*sizeof(real_T));
		}
	}
